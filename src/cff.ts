...
export type prop = typeof prop[keyof typeof prop];
...
const SID = Object.fromEntries(SID_NAMES.map((name, i) => [name, i])) as { [K in typeof SID_NAMES[number]]: number };
type SID = typeof SID[keyof typeof SID];
...
//	static get(file: binary._stream) { return new this(file); }
	...
	getString(sid: number) {
		return sid < SID_NAMES.length ? SID_NAMES[sid] : this.strings[sid - SID_NAMES.length];
	}
